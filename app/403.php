<?php require 'pages/error/header.php' ?>
<title>Forbidden</title>

</head>
<body data-gr-c-s-loaded="true">
<div class="wrapper">
    <div class="container">
        <?php echo header_image(); ?>

        <div class="content-primary">
            <h1 class="title">Forbidden</h1>
            <p class="description">The page or folder you are trying to view is not accessible.</p>
            <div class="section-footer">
                <a href="https://www.gollahalli.me" class="button button-primary">Back To Homepage</a>
                <a href="mailto:akshay@gollahalli.me?Subject=Forbidden%20error" class="button button-default">Report Error</a>
            </div>
        </div>
    </div>

<?php require 'pages/error/footer.php' ?>